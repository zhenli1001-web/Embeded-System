...
static uint16_t Joystick_ReadChannel(Joystick_cfg_t* cfg, uint32_t channel)
{
    cfg->adc_config.Channel = channel;
    if (HAL_ADC_ConfigChannel(cfg->adc, &cfg->adc_config) != HAL_OK) {
        return 0U;
    }

    // Discard the first sample after a channel switch so the ADC sample-and-hold
    // capacitor can settle when multiple potentiometer channels share one ADC.
    if (HAL_ADC_Start(cfg->adc) != HAL_OK) {
        return 0U;
    }
    if (HAL_ADC_PollForConversion(cfg->adc, HAL_MAX_DELAY) != HAL_OK) {
        HAL_ADC_Stop(cfg->adc);
        return 0U;
    }
    (void)HAL_ADC_GetValue(cfg->adc);
    HAL_ADC_Stop(cfg->adc);
...
{
    data->x_raw = Joystick_ReadChannel(cfg, cfg->x_channel);
    data->y_raw = Joystick_ReadChannel(cfg, cfg->y_channel);
